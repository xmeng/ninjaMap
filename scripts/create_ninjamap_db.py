...
def process_raw_seedfile(seedfile):
    ...
    df["Strain_Name"] = df["Strain_Name"].apply(
...
def download_genome(genome_name, reference_id, ftp_link, fasta_dir, metadata_dir):
    ...
    if ftp_link.startswith("s3://"):
        ...
        try:
            ...
            error_msg = "\n".join(sys.exc_info())
            print(
                f"[ERROR] Unable to retrieve reference from URL {ftp_link}\nThe following error occurred: {error_msg}"
            )
            sys.exit(1)

    outfasta, outbin, outmeta = standardize_sequence_names(
        name=genome_name,
...
def standardize_sequence_names(name, reference_id, localfile, fasta_dir, metadata_dir):
    ...
    outmeta.write(
        "AssemblyID\tGenomeName\tNewSeqHeaders\tOriginalSeqHeaders\tSeqLen\tNum_Ns\n"
    )
    ...
    try:
        with gzip.open(localfile, "rt") as handle:
            for index, record in enumerate(SeqIO.parse(handle, "fasta")):
                ...
                outmeta.write(
                    f'{reference_id}\t{name}\t{header}\t{record.id}\t{len(record.seq)}\t{record.seq.count("N")}\n'
                )
        ...
        error_msg = "\n".join(sys.exc_info())
        print(
            f"[ERROR] Could not process fasta file for {name} from {localfile}\n{error_msg}"
        )
    else:
        os.remove(localfile)
    finally:
...
def generate_db_summary(genome_metadata_file, db_metadata_file):
    metadata = (
        pd.read_table(genome_metadata_file)
        .groupby("GenomeName")
        .agg(
...
if __name__ == "__main__":
    ...
    # Download Genomes
    try:
        db_files = genomes_df.apply(
            lambda row: download_genome(
                genome_name=row["Strain_Name"],
    ...
    # Concatenate data
    concatenate_files(
        output_file=f"{output_dir}/{db_name}.fna",
        file_list=db_files["fastafiles"],
        has_header=False,
        cleanup=False,
    )
    concatenate_files(
        output_file=f"{output_dir}/{db_name}.nm_mates_bin.tsv",
        file_list=db_files["binfiles"],
        has_header=False,
        cleanup=cleanup,
    )
    concatenate_files(
        output_file=f"{output_dir}/{db_name}.per_genome_metadata.tsv",
        file_list=db_files["metafiles"],
        has_header=True,
        cleanup=cleanup,
    )

    generate_db_summary(
        f"{output_dir}/{db_name}.per_genome_metadata.tsv",
        f"{output_dir}/{db_name}.db_metadata.tsv",
    )
