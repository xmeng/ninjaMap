...
# Optional
p.add_argument("-prefix", dest="prefix", action="store", type=str, help="output prefix")
p.add_argument(
    "-threads",
    dest="threads",
    action="store",
...
if args["debug"]:
    ...
    false_positives = pysam.AlignmentFile(
        prefix + ".ninjaMap.false_positives.bam", "wb", template=tmp_bamfile
    )
    true_positives = pysam.AlignmentFile(
        prefix + ".ninjaMap.true_positives.bam", "wb", template=tmp_bamfile
    )
...
class Strains:
    ...
    def add_contig(self, contig_name, contig_length):
        ...
        self.contigs_covered[contig_name] = np.zeroes(int(contig_length))
        self.genome_size += int(contig_length)
        self.num_contigs += 1

    def add_paired_singular_vote(
        self, read_name, mate_name, read_template_len, read_vote=1
    ):
    ...
    def calculate_singular_coverage(self, bamfile_name):
        ...
        self.uniquely_covered_depth, self.uniquely_covered_bases = self._calc_coverage(
            bamfile_name, self.singular_bin
        )
        ...
        return (
            self.uniquely_covered_depth,
            self.name,
            self.covered_bases,
            self.total_covered_depth,
            self.weighted_base_depth,
        )  # 2147483647, self.covered_bases) #unique_contig_name)
    ...
    def calculate_escrow_coverage(self, bamfile_name):
        ...
        (
            contig_depths,
            escrow_genome_depth,
            escrow_genome_coverage,
        ) = self._calc_coverage(bamfile_name, self.escrow_bin, self.contigs)

        return (
            self.escrow_covered_depth,
            self.name,
            self.covered_bases,
            self.total_covered_depth,
            self.weighted_base_depth,
    ...
    @staticmethod
    def _calc_coverage(bamfile_name, contigs_dict, read_dict):
        """
        Motive:
            Calculate the depth and coverage for each contig in a genome
        Accept: 
            bamfile_name 
            contigs_dict {contig_names : contigs_length} for this strain
            read_dict {unique_read_name : read_weight} reads selected for this strain
        Returns a tuple of 3, in order:
            contig_depths {contig_name: 1d numpy array of total depths at per base}
            wavg_genome_depth (int) : weighted average of adjusted depth (= contig depth * median read weight)
                of all contigs by contig length
            genome_coverage (int): number of bases with depth greater than 0 across all contigs
        """
        read_weights = list()

        def keep_read(aln, bin_dict):
            """
            Motive: 
                Callback function used by PySam's count coverage function 
                to only return coverage for reads that retuen True here.
            Accept: 
                aln - alignment object from PySam count_coverage
                bin_dict = read_dict
            Return: 
                Required by PySam to ONLY return a boolean value [True/False]
            NOTE:
                Since PySam will only allow a boolean to be returned
                this function also updates read_weights array with values from read_dict, 
                read_weights array is initiated outside this function, so as to use later.
            """
            read_unique_name = Reads.get_unique_read_name(aln)
            if read_unique_name in bin_dict:
                read_weights.append(bin_dict[read_unique_name])
                return True
            else:
                return False

        check_read = partial(keep_read, bin_dict=read_dict)

        contig_weights = dict()
        contig_depths = dict()
        genome_coverage = 0
        wavg_genome_depth = 0
        adjusted_depth_array = list()
        contig_length_array = list()
        # stats per contig
        with pysam.AlignmentFile(bamfile_name, mode="rb") as cov_bamfile:
            for contig_name, contig_length in contigs_dict.items():
                cov_counts = cov_bamfile.count_coverage(
                    contig_name,
                    start=0,
                    end=contig_length,
                ...
                per_base_depth = np.nansum(cov_counts, axis=0)
                total_depth = np.sum(per_base_depth)
                #
                logging.info(
                    f"{contig_name}|{contig_length}|{cov_counts.shape}|{per_base_depth.shape}"
                )
                sys.exit(0)
                #
                median_read_weight = 0
                if len(read_weights) > 0:
                    median_read_weight = np.nanmedian(np.array(read_weights))
                read_weights = list()  # re-initialize for next contig

                contigs_depths.update({contig_name: per_base_depth})
                # contigs_weights.update({contig_name: np.array(read_weights)})
                contigs_weights.update({contig_name: median_read_weight})
                genome_coverage += len(per_base_depth[per_base_depth > 0])

                adjusted_depth_array.append(total_depth * median_read_weight)
                contig_length_array.append(contig_length)

        wavg_genome_depth = np.average(
            adjusted_depth_array, weigths=contig_length_array
        )
        return (contigs_depths, wavg_genome_depth, genome_coverage)
    ...
    def _normalize_votes(self):
        ...
        if self.num_singular_reads > 0:
            self.singular_vote_conversion_rate = (
                self.cum_primary_votes / self.num_singular_reads
            )
        else:
            self.singular_vote_conversion_rate = 0

        if self.num_escrow_reads > 0:
            self.escrow_vote_conversion_rate = (
                self.cum_escrow_votes / self.num_escrow_reads
            )
        else:
            self.escrow_vote_conversion_rate = 0
    ...
    def calculate_read_fraction(self):
        self.read_fraction = (
            (self.cum_escrow_votes + self.cum_primary_votes)
            * 100
...
class Reads:
    ...
    def get_voting_details(self, approved_strain_list):
        ...
        for strain in approved_strain_list:
            ...
            vote_list.append(
                [strain_name, singular_votes, escrow_votes, cumulative_vote]
            )
        return vote_list
    ...
    def choose_primary_candidate(read, mate):
        ...
        if read.mate_has_perfect_match or mate.mate_has_perfect_match:
            common_strains_list = intersection(
                read.mapped_strains.keys(), mate.mapped_strains.keys()
            )
    ...
    def check_for_fraud(votes_file, discard_pile=1):
        ...
        df = pd.read_csv(
            votes_file,
            index_col="Read_Name",
            usecols=["Read_Name", "cSingular_Vote", "cEscrow_Vote"],
        )
        ...
        mid_row, mid_col = votes_df[
            (votes_df.Total_Votes > 0.001) & (votes_df.Total_Votes < 0.999)
        ].shape
...
logging.info(
    "\t%d contigs assigned to %d strains", len(bins.keys()), len(all_strain_obj.keys())
)
###############################################################################
# Parse the BAM file
###############################################################################
...
logging.info(
    "\tUsed %d reads with perfect alignments, out of %d (%7.3f%%).",
...
logging.info(
    "\tUsed %d reads for primary distribution, out of %d (%7.3f%%) reads with perfect alignments or %7.3f%% of total.",
...
if len(read_objects.keys()) != Reads.reads_w_perfect_alignments:
    logging.critical(
        "Read %d reads with perfect alignments, but created %d read objects. There is something fishy going on here...",
        Reads.reads_w_perfect_alignments,
...
# Header
votes.write(
    "Read_Name,Strain_Name,cSingular_Vote,cEscrow_Vote,Num_Strains_Aligned,was_discarded,has_voted,is_singular,mate_has_perfect_aln\n"
)
...
for name, read in read_objects.items():
    ...
    if read.mate_has_perfect_match:
        ...
        if strain_name is not None:
            strain = all_strain_obj[strain_name]
            strain.add_paired_singular_vote(
                read.unique_name, mate.unique_name, read.template_length, 1
            )
            read.add_vote(1)
            mate.add_vote(1)
            ...
            votes.write(
                ...
                + strain.name
                + ","
                + str(strain.singular_bin[read.unique_name])
                + ","
                + str(strain.escrow_bin[read.unique_name])
                + ","
                + "1,False,"
                + str(read.has_voted)
                + ","
                + str(read.in_singular_bin)
                + ","
                + str(read.mate_has_perfect_match)
                + "\n"
            )
            ...
            votes.write(
                ...
                + strain.name
                + ","
                + str(strain.singular_bin[mate.unique_name])
                + ","
                + str(strain.escrow_bin[mate.unique_name])
                + ","
                + "1,False,"
                + str(mate.has_voted)
                + ","
                + str(mate.in_singular_bin)
                + ","
                + str(read.mate_has_perfect_match)
                + "\n"
            )
...
logging.info(
    "\t%d reads will be used for singular alignment strain abundance out of %d (%7.3f%%) reads with perfect alignments or %7.3f%% of total aligned.",
...
logging.info(
    "\t%d reads for escrow alignment strain abundance out of %d (%7.3f%%) reads with perfect alignments or %7.3f%% of total aligned.",
...
if Reads.reads_w_perfect_alignments != (
    Reads.total_escrow_reads + Reads.total_singular_reads_in_pairs
):
    logging.critical(
        "Read %d read objects with perfect alignments, but created %d read objects as singular and escrow total (%d + %d). There is something fishy going on here...",
        Reads.reads_w_perfect_alignments,
        (Reads.total_escrow_reads + Reads.total_singular_reads_in_pairs),
        Reads.total_singular_reads_in_pairs,
        Reads.total_escrow_reads,
...
###############################################################################
logging.info(
    "Computing depth and coverage for each strain in the database based on singular alignments ..."
)
...
for name, strain in all_strain_obj.items():
    i += 1
    logging.info(
        "\t[%d/%d] Searching for exclusive support for :\t%s",
        i,
    ...
    result.append(
        pool.apply_async(strain.calculate_singular_coverage, args=(bamfile_name,))
    )
pool.close()
pool.join()
...
for res in result:
    ...
    if not (res.get() is None):
        (
            myuniquely_covered_depth,
            mystrain_name,
            mycovered_length,
            mytotal_covered_depth,
            myweighted_base_depth,
        ) = res.get()
        if not (myuniquely_covered_depth is None):
            all_strain_obj[
                mystrain_name
            ].uniquely_covered_depth += myuniquely_covered_depth
        if not (mycovered_length is None):
            all_strain_obj[
                mystrain_name
            ].covered_length = mycovered_length  # .covered_bases.update( mycovered_bases_set ) #.add(mycovered_bases)
    ...
    for name, read in escrow_read_objects.items():
        ...
        if not read.has_voted:
            ...
            for strain in common_strains_list:
                ...
                adj_singular_vote[
                    ...
                    strain.mike_drop_adjustment()
                )  # self.cum_primary_votes / self.indexed_uniquely_covered_bases
            ...
            if total_adj_singular_votes > 0:
                ...
                for strain, adj_strain_singular_vote in adj_singular_vote.items():
                    read_vote_value = (
                        1 * adj_strain_singular_vote / total_adj_singular_votes
                    )
                    strain.add_escrow_vote(
                        read.unique_name, read.read_length, read_vote_value
                    )
        ...
        for voting_detail_sublist in voting_details:
            (
                ...
                escrow_vote_count,
                cumulative_vote,
            ) = voting_detail_sublist
            votes.write(
                read.name
                + ","
                + strain_name
                + ","
                + str(singular_vote_count)
                + ","
                + str(escrow_vote_count)
                + ","
                + str(len(common_strains_list))
                + ","
                + str(to_discard)
                + ","
                + str(read.has_voted)
...
logging.info(
    "\tUsed %d reads for escrow distribution, out of %d (%7.3f%%) reads with perfect alignments or %7.3f%% of total.",
    Reads.total_escrow_reads,
    Reads.reads_w_perfect_alignments,
    Reads.total_escrow_reads * 100 / Reads.reads_w_perfect_alignments,
    Reads.total_escrow_reads * 100 / Reads.total_reads_aligned,
...
if Reads.total_escrow_reads_discarded > 0:
    logging.info(
        "\t%d out of %d escrow reads had to be discarded because their singular weight was too low, but %d (%7.3f%%) were still retained",
...
if singular_fraud_alert or escrow_fraud_alert:
    logging.critical(
        "[FATAL] There were signs of voter fraud. See the votes file (%s) for the complete picture.",
        vote_file,
    )
    logging.critical(
        "\tVoter fraud committed by "
        + str(num_singular_fraud_reads)
        + " out of "
        + str(Reads.total_singular_reads_in_pairs)
        + " singular reads in pairs"
    )
    logging.critical(
        "\tVoter fraud committed by "
        + str(num_fraud_reads)
        + " out of "
        + str(Reads.total_escrow_reads)
        + " escrow reads"
    )
...
logging.info(
    "Computing depth and coverage for each strain in the database based on escrow alignments ..."
)
...
for name, strain in all_strain_obj.items():
    i += 1
    logging.info(
        "\t[%d/%d]Searching for escrow support for :\t%s",
        i,
        Strains.total_strains,
        name,
    )
    ...
    result2.append(
        pool2.apply_async(strain.calculate_escrow_coverage, args=(bamfile_name,))
    )
pool2.close()
pool2.join()
    ...
    if not (res.get() is None):
        ...
        if not (mycovered_length is None):
            all_strain_obj[
                ...
                mycovered_length  # .covered_bases.update( mycovered_bases_set2 )
            )
        if not (mytotal_covered_depth is None):
            all_strain_obj[mystrain_name].total_covered_depth = mytotal_covered_depth
        if not (myweighted_base_depth is None):
...
# Header
stats.write(
    "File_Name,Reads_Aligned,Reads_wPerfect_Aln,Reads_wSingular_Votes,Reads_wEscrowed_Votes,Discarded_Reads_w_Perfect_Aln\n"
)
stats.write(
    prefix
    + ","
...
# timem python ninjamap_p.py \
#     -threads 15 \
#     -bam SCV2-Week5-Mouse4/bowtie2/SCV2-Week5-Mouse4.sortedByCoord.bam \
#     -bin db/SCv2_2.ninjaIndex.binmap.csv \
#     -outdir ninjaMapP_v2 \
#     -prefix  SCV2-Week5-Mouse4 | \
#     tee -a Log/SCV2-Week5-Mouse4.ninjaMapP.log.txt
