...
def save_predictions(
    ...
    stats_df["Prediction"] = stats_df.apply(
        lambda row: get_extreme_prediction(row["Extreme_Positions"]), axis=1
    )

    stats_df["missed_2_EP"] = stats_df.apply(
        lambda row: invader_probability(
            row["Pos_Selected_Found"], row["Pos_Selected_Searched"], 2
        ),
        axis=1,
    )
...
def adjust_predictions(current_pred, missed_2_EP, cum_read_supp_0):
    adj_pred = "Unclear"
    if current_pred.lower() == "invader":
        ...
        and (missed_2_EP < 0.05)
        #  Nucl_Read_Support_Total_at_0 > 1000
        and (cum_read_supp_0 >= 1000)
    ):
        adj_pred = "Input"
...
def get_extreme_prediction(num_extreme_positions):
    prediction = "Unclear"
