...
mkdir -p "${OUTPUTDIR}" "${LOCAL_OUTPUT}" "${LOG_DIR}" "${RAW_FASTQ}" "${QC_FASTQ}" "${MINIMAP2_OUTPUT}" "${SNP_OUTPUT}"
mkdir -p "${ASSEMBLY_OUTPUT}" "${QUAST_OUTPUT}" "${FASTQC_OUTPUT_1}" "${FASTQC_OUTPUT_2}" "${BAMQC_OUTPUT}" "${BAMQC_OUTPUT2}" "${BAMQC_OUTPUT3}"
...
bwa mem -t ${coreNum} "${ASSEMBLY_OUTPUT}/contigs.fasta" "${BAMQC_OUTPUT}/read1_filtered.fastq.gz" "${BAMQC_OUTPUT}/read2_filtered.fastq.gz" > "${BAMQC_OUTPUT}/reads_aligned_contigs.sam"
...
cp "${ASSEMBLY_OUTPUT}/filtered_contigs_1kb.fasta"  ${MINIMAP2_OUTPUT}/${DIR_NAME}_contigs_1kb.fasta
cp  "${RAW_FASTQ}/insertion.fa"  ${MINIMAP2_OUTPUT}/${DIR_NAME}_insertion.fasta
minimap2 -t ${coreNum} -a "${ASSEMBLY_OUTPUT}/filtered_contigs_1kb.fasta"  "${RAW_FASTQ}/insertion.fa" | samtools sort -@${coreNum} -o "${MINIMAP2_OUTPUT}/${DIR_NAME}_insertion_vs_contigs.sorted.bam" -
...
: <<'END2'
#Rapid haploid variant calling and core genome alignment --mincov 3 \ --minfrac 0.5 \
snippy --cpus ${coreNum} \
--report \
--force \
--outdir "${SNP_OUTPUT}" \
...
-r "${RAW_FASTQ}/ref_combination.fa" \
