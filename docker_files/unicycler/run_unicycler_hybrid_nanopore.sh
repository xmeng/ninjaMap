...
if [ $ShortTargetBase -gt 990000000 ]
then
   ShortTargetBase=990000000
fi

if [ $LongTargetBase -gt 990000000 ]
then
   LongTargetBase=990000000
fi
...
echo "Reads Filtering" >> ${LOG_DIR}/bbtools.log.txt
echo "**************************" >> ${LOG_DIR}/bbtools.log.txt
...
# clean data
rm "${BWT_OUTPUT}/contigs_aligned.sam" "${BWT_OUTPUT}/contigs_aligned.bam"
...
echo 'Sample_Name,Total_LongReads,Total_ShortReads,Total_contigs,longAfterFiltering,shortAfterFiltering,long_mapping,short_mapping,Assembly_status' > ${LOG_DIR}/read_stats.csv
echo ${SAMPLE_NAME}','${totalLongReads}','${totalShortReads}','${totalContigs}','${longAfterTrim}'('$longUsedRate'%),'${shortAfterTrim}'('$shortUsedRate'%),'${long_mapping}','${short_mapping}','${status} >> ${LOG_DIR}/read_stats.csv
