...
echo 'Sample_Name,Total_LongReads,Total_ShortReads,Total_contigs,longAfterSampling,shortAfterSampling,long_mapping,short_mapping,AvgLongReadLength,Assembly_status' > ${LOG_DIR}/read_stats.csv
echo ${SAMPLE_NAME}','${totalLongReads}','${totalShortReads}','${totalContigs}','${longAfterTrim}'('$longUsedRate'%),'${sampledReads}'('$shortUsedRate'%),'${long_mapping}','${short_mapping}','${AvgLongReadLength}','${status} >> ${LOG_DIR}/read_stats.csv
