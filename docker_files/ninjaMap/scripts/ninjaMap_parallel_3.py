...
#         print(str(self) +'\t:\t' + self.name +'\t:\t' + read.unique_name +'\t:\t'+ str(read_vote))
    ...
    def calculate_singular_coverage (self, bamfile_name):
        ...
        return (self.uniquely_covered_depth, self.name, output_baseset_filename, self.total_covered_depth, self.weighted_base_depth )
...
#             for pileupcolumn in cov_bamfile.pileup(contig = contig_name, stepper = 'samtools', fastafile = fasta, min_base_quality = min_base_qual):
        ...
        return (self.escrow_covered_depth, self.name, input_baseset_filename, self.total_covered_depth, self.weighted_base_depth) #, self.covered_bases) #mycovered_bases )
