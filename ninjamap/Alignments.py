...
class Alignments:
    ...
    def __init__(self, aln, paired):
        ...
        self.read_base = (
            self.read_name
        ) = self.mate_name = self.orientation = self.reference_name = ""
        ...
        if aln.has_tag("NM"):
            ...
            (
                ...
                self.recall,
                self.f_score,
            ) = self.extract_aln_info(aln)
        else:
            self.is_aln = False

    def is_valid_alignment(
        self, min_perc_id, min_perc_aln, max_perc_indel, min_f_score
    ):
        """
    ...
    def extract_aln_info(self, aln):
        ...
        qlen = (
            aln_match + insertions + BAM_CSOFT_CLIP + BAM_CHARD_CLIP + match + mismatch
        )
        if qlen != read_length:
            print(
                f"Calculated query length({qlen}) is not equal to the given query length:{read_length}"
            )
            sys.exit(1)
        ...
        if match > 0:
            ...
            f_score = Alignments.f_beta_score(
                precision, recall, 2
            )  # f2 score to weigh recall higher
    ...
    def f_beta_score(precision, recall, beta=1):
        beta_sq = (beta) ** 2
        f_beta = (
            (1 + beta_sq) * (precision * recall) / float((beta_sq * precision) + recall)
        )
        return f_beta
