...
def usage():
    ...
    # Optional
    p.add_argument(
        "-prefix", dest="prefix", action="store", type=str, help="output prefix"
    )
    p.add_argument(
        "-ccs",
        dest="ccs",
...
if __name__ == "__main__":
    ...
    logging.info(
        "\t%d contigs assigned to %d strains",
        len(bins.keys()),
        len(all_strain_obj.keys()),
    )
    ###############################################################################
    # Parse the BAM file
    ###############################################################################
    ...
    if total_reads_aligned == 0:
        logging.critical(
            f"{total_reads_aligned} reads aligned to the reference database. Please check the BAM file or the database used.",
        )
        sys.exit(1)
    ...
    logging.info(
        f"\tFound {total_alignments} alignments for {total_reads_aligned} reads. Of which, {bad_aln} were invalid alignments"
    )
    logging.info(
        f"\t{valid_alignments} alignments (~{aln_passed_percent}%) passed the acceptable alignment criteria."
    )
    logging.info(
        f"\tAcceptable alignments belonged to {reads_w_valid_alignments} reads (~{read_passed_percent}%)."
    )
    ###############################################################################
    # The End
    ###############################################################################
